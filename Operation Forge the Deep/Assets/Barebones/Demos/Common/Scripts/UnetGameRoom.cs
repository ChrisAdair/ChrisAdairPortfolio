...
public class UnetGameRoom : NetworkBehaviour
{
    ...
    public string GrainStructurePath = null;
    public string LevelID = "1";
    public string ChatChannel = null;
    ...
    protected virtual void BeforeRegisteringRoom()
    {
        if (SpawnTaskController != null)
        {
            ...
            if (prop.ContainsKey("GameStructurePath"))
            {
                GrainStructurePath = prop["GameStructurePath"];
                Logger.Debug("Grain Path Name: " + GrainStructurePath);
            }
            else
                Logger.Debug("Grain Path Name does not exist in dictionary");

            if (prop.ContainsKey("LevelID"))
            {
                LevelID = prop["LevelID"];
            }

            if (prop.ContainsKey("ChatChannel"))
            {
                ChatChannel = prop["ChatChannel"];
            }
            
            if(prop.ContainsKey("ZoneID"))
            {
                ZoneID = prop["ZoneID"];
            }
        }
    ...
    public virtual void RegisterRoom()
    {
        ...
        var options = new RoomOptions()
        {
            ...
            Properties = new Dictionary<string, string>()
            {
                ...
                {"GrainStructurePath", GrainStructurePath },
                {"LevelID", LevelID }
            }
        };
